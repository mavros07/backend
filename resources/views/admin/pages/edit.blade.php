@push('body-end')
  ...
  <div id="media-modal" class="fixed inset-0 z-[220] hidden items-center justify-center bg-black/60 p-4">
    <div class="mx-auto flex h-full max-h-[85vh] w-full max-w-[min(64rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-lg bg-white shadow-xl">
    ...
    <form method="post" action="{{ route('admin.pages.update', ['slug' => $slug]) }}" class="space-y-6">
      ...
      <div class="grid gap-6 lg:grid-cols-12 lg:items-start">
        <aside class="order-1 space-y-6 lg:order-2 lg:col-span-4">
          <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm lg:sticky lg:top-6">
            <div class="border-b border-gray-100 bg-gray-50/80 px-5 py-4">
              <h3 class="text-sm font-semibold uppercase tracking-wide text-gray-500">{{ __('Publishing') }}</h3>
            </div>
            <div class="p-5">
              <label class="flex cursor-pointer items-start gap-3 rounded-lg border border-gray-200 bg-gray-50/50 p-4 transition hover:border-indigo-200 hover:bg-indigo-50/30">
              ...
              <div class="mt-6 border-t border-gray-100 pt-5">
                <button type="submit" class="inline-flex w-full items-center justify-center rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                  {{ __('Save page') }}
                </button>
              </div>
            </div>
