              ...
              <thead>
                <tr class="text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                  ...
                  @if($isAdminList)
                    ...
                    <th class="whitespace-nowrap px-4 py-3">{{ __('Created') }}</th>
                ...
                @foreach($vehicles as $vehicle)
                  ...
                  <tr class="align-top">
                    ...
                    @if($isAdminList)
                      ...
                      <td class="whitespace-nowrap px-4 py-3 text-slate-600">{{ $vehicle->created_at?->format('M j, Y g:i a') }}</td>
                      <td class="whitespace-nowrap px-4 py-3 text-slate-600">{{ $vehicle->submitted_at?->format('M j, Y g:i a') ?? '—' }}</td>
                    @else
                      <td class="whitespace-nowrap px-4 py-3 text-slate-600">{{ $vehicle->created_at?->format('M j, Y') }}</td>
                    @endif
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 text-right">
                      <div class="flex flex-col items-end gap-2 sm:flex-row sm:flex-wrap sm:justify-end">
                        @if($vehicle->status === 'approved')
                          <a href="{{ route('inventory.show', ['slug' => $vehicle->slug]) }}" class="text-sm font-medium text-slate-600 hover:text-amber-600">{{ __('View') }}</a>
                        @endif
                        <a href="{{ route('dashboard.vehicles.edit', $vehicle) }}" class="text-sm font-medium text-amber-700 hover:text-amber-900">{{ __('Edit') }}</a>
                        @if($isAdminList)
                          @if(in_array($vehicle->status, ['pending', 'draft', 'rejected'], true))
                            <form method="post" action="{{ route('admin.vehicles.approve', $vehicle) }}" class="inline">
                              @csrf
                              <button type="submit" class="text-sm font-medium text-emerald-700 hover:underline">{{ __('Approve') }}</button>
                            </form>
                          @endif
                          @if($vehicle->status !== 'rejected')
                            <details class="text-left">
                              <summary class="cursor-pointer text-sm font-medium text-rose-700 hover:underline">{{ __('Reject…') }}</summary>
                              <form method="post" action="{{ route('admin.vehicles.reject', $vehicle) }}" class="mt-2 w-56 space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-left shadow">
                                @csrf
                                <label class="block text-xs font-medium text-slate-600">{{ __('Reason (optional)') }}</label>
                                <textarea name="rejection_reason" rows="2" class="w-full rounded-md border-slate-300 text-xs shadow-sm" placeholder="{{ __('Explain to the vendor…') }}">{{ old('rejection_reason', $vehicle->rejection_reason) }}</textarea>
                                <button type="submit" class="w-full rounded-md bg-rose-600 px-2 py-1.5 text-xs font-semibold text-white hover:bg-rose-500">{{ __('Reject listing') }}</button>
                              </form>
                            </details>
                          @endif
                          <form method="post" action="{{ route('dashboard.vehicles.destroy', $vehicle) }}" class="inline" onsubmit="return confirm(@json(__('Delete this listing permanently?')));">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-sm font-medium text-rose-800 hover:underline">{{ __('Delete') }}</button>
                          </form>
                        @else
                          <form method="post" action="{{ route('dashboard.vehicles.destroy', $vehicle) }}" class="inline" onsubmit="return confirm(@json(__('Delete this listing permanently?')));">
                            @csrf
